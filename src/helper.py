...
from src.cuckoo_filter_ai import CuckooFilter
...
SAVED_BLOOM_FILTER = DATA_DIR + "saved_bloom_filter.pkl"
SAVED_CUCKOO_FILTER = DATA_DIR + "saved_cuckoo_filter.pkl"
...
def run_cuckoo_filter(dataset):
    print("Running demo of Cuckoo Filter...")

    # Build the data structure from text file
    build_cuckoo_filter_from_file(resamble_filename(SORTED_USERNAMES_FILE, dataset))

    # Load save binary data structure into memory
    cf = load_filter_from_disk(resamble_filename(SAVED_CUCKOO_FILTER, dataset, True))
...
def prepare_data_for_simple_search(dataset):
    ...
    with open(resamble_filename(SORTED_USERNAMES_FILE, dataset), "r") as f:
        step = int(1 / 1)
        name_list = [line.strip() for line in islice(f, 0, None, step)]
